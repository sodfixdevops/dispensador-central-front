...
import { ReporteTotalesGeneralesResponseDto } from "@/app/lib/definitions";
...
export default function ReporteTotalesGeneralesPage() {
  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        const t = res.data.totales?.[0];
        setCantidad(Number(t?.cantidadBilletes || 0));
        setImporte(Number(t?.importe || 0));
  ...
  const exportarExcel = () => {
    ...
    const data = [
      ["", "TOTALES GENERALES", "", ""],
      ["", "MO", "CANTIDAD", "IMPORTE"],
      ["TOTAL DEPOSITADO", "BS", cantidad, Number(importe).toFixed(2)],
    ];
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse border border-gray-300 text-sm">
              <tbody>
                <tr className="bg-[#123d6b] text-white font-bold text-center">
                ...
                <tr className="bg-gray-100 font-semibold text-center">
                  ...
                  <td className="border border-gray-300 px-4 py-2">CANTIDAD</td>
                  <td className="border border-gray-300 px-4 py-2">IMPORTE</td>
                </tr>
                <tr className="text-center font-semibold">
                  <td className="border border-gray-300 px-4 py-2">TOTAL DEPOSITADO</td>
                  <td className="border border-gray-300 px-4 py-2">BS</td>
                  <td className="border border-gray-300 px-4 py-2">{cantidad}</td>
                  <td className="border border-gray-300 px-4 py-2">{importe.toLocaleString("es-BO", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                </tr>
              </tbody>
            </table>
          </div>
